<?php
    ...
    //recupero iscrizioni effettuate dallo studente loggato
    $parametri=array(
        $matricola
    );
    $sql="SELECT * FROM \"Unidb\".iscrizione WHERE matricola=$1";
    $risultato=pg_prepare($db,"vis_iscrizioni_studente",$sql);
    $risultato=pg_execute($db,"vis_iscrizioni_studente",$parametri);
    ...
    if($risultato){
        ...
        while($row=pg_fetch_assoc($risultato)){
            ...
            $iscrizioni[$i]=array($id_corso,$id_insegnamento,$id_docente,$data_esame);
            $i++;
        }
    }
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container mt-5 border">
        <h3 class="text-center">Iscrizioni Studente</h3>
        </br>
        <?php 
        ...
        <?php
        // Visualizzo i record
        foreach($iscrizioni as $i=>$valori){
        ?>
            <tr>
