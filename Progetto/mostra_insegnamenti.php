<?php
    ...
    //recupero informazioni insegnamenti
    $insegnamenti=array();
    $sql="SELECT insegnamento.id,insegnamento.id_responsabile,insegnamento.nome,insegnamento.anno,insegnamento.descrizione
    FROM \"Unidb\".insegnamento;";
    
    $parametri=array();

    $risultato=pg_prepare($db,"vis_insegnamenti",$sql);
    $risultato=pg_execute($db,"vis_insegnamenti",$parametri);
    ...
    if($risultato){
        ...
        while($row=pg_fetch_assoc($risultato)){
            ...
            $insegnamenti[$i]=array($id,$id_responsabile,$nome,$anno,$descrizione);
            $i++;
        }
    }
...
<html lang="en">
  ...
  <body>
    ...
    <div class="container mt-5 border">
        <h3 class="text-center mt-3">Insegnamenti</h3>
        </br>
        <?php 
        // In caso di mancanza di risultati
        if (count($insegnamenti) == 0) {?>
            <div class="alert alert-warning" role="alert">
                <p>Nessuno insegnamento trovato</p>
            </div>
        <?php }
        ...
        <?php
        // Visualizzo i record
        foreach($insegnamenti as $i=>$valori){
        ?>
            <tr>
