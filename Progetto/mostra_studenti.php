<?php
    ...
    //recupero informazioni studenti attivi
    $studenti_attivi=array();
    $sql="SELECT studente.matricola,studente.id_corso,studente.nome,studente.cognome,studente.nome_utente,studente.anno_immatricolazione
    FROM \"Unidb\".studente;";
    
    $parametri=array();

    $risultato=pg_prepare($db,"vis_studenti_attivi",$sql);
    $risultato=pg_execute($db,"vis_studenti_attivi",$parametri);
    ...
    if($risultato){
        ...
        while($row=pg_fetch_assoc($risultato)){
            ...
            $studenti_attivi[$i]=array($matricola,$id_corso,$nome,$cognome,$nome_utente,$anno_immatricolazione);
            $i++;
        }
    }
...
<html lang="en">
  ...
  <?php
    ...
    <div class="container mt-5 border">
        <h3 class="text-center">Studenti Attivi</h3>
        </br>
        <?php 
        // In caso di mancanza di risultati
        if (count($studenti_attivi) == 0) {?>
            <div class="alert alert-warning" role="alert">
                <p>Nessuno studente attivo trovato</p>
            </div>
        <?php }
        ...
        <?php
        // Visualizzo i record
        foreach($studenti_attivi as $i=>$valori){
        ?>
            <tr>
